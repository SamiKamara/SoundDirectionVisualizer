...
8. Confirm quiet-device audio remains visible, center shows front/back, side pans spread away from that axis after several active frames, and history expires.
9. Confirm the game retains keyboard and mouse focus while the overlay is visible.
10. Confirm color, percentage opacity, display-height size, dimensions, offsets, labels, and trail settings preview immediately while editing.
11. Press Cancel and confirm the previously saved appearance is restored; reopen settings, change values, press Save, and confirm they persist.
12. Toggle the ring, cardinal ticks, current rays, current markers, listener dot, trail, and labels independently; confirm only the selected layers remain.
13. Confirm overlay toggle and settings hotkeys work globally.
14. Confirm manual display selection and display cycling on a multi-monitor system.
15. With Steam available, move a borderless game between displays and confirm auto targeting follows it.
16. Turn automatic display targeting off while preferred game audio remains on; confirm the overlay stays on the manual display while audio still follows the game process.
17. Disconnect/reconnect a display and confirm the app falls back without exiting.
18. Disable automatic calibration and confirm the manual silence-threshold and hard-pan controls become available and retain the legacy fixed behavior.
19. Select a non-stereo endpoint, disable direct game capture, and confirm a clear error appears without a crash.
20. Launch a second app instance and confirm it reopens the existing settings window.
21. Exit from the tray and confirm capture, overlay, hotkeys, and notification icon stop.
